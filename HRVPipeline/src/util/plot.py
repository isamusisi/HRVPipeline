...
def plot_ppg(ppg, title='ppg'):
    ...
    ppg.plot(y='peaks', color='blue', ax=ax, label=f"Peaks {ppg['peaks'].to_list().count(1)}")
    p.legend(loc='best')
    p.show()


def plot_snirf_ppg(ppg, title='ppg'):

    p.plot(ppg.time, ppg['PPG'], label='PPG')
    p.plot(ppg.time, ppg['peaks'], label='Peaks')
    p.title(title)
